---
description: Erfahren Sie, wie Sie Fragebögen und DSFAs mit Dastra durchführen.
---

# Fragebögen

Das Modul Fragebögen von Dastra ermöglicht es Ihnen, alle für die Steuerung Ihrer Compliance erforderlichen Informationen zu erfassen: DSFA, Fragebögen für Auftragsverarbeiter, Compliance-Fragebögen, Transferfolgenabschätzungen und vieles mehr.

Um auf das Modul zuzugreifen, klicken Sie auf das Symbol **"Fragebögen"** auf der linken Seite des Bildschirms. Das Modul ist in drei Registerkarten gegliedert: **Fragebogenvorlagen**, **Fragebögen anzeigen** und **Meine Antworten**.

## Benutzer-Tutorial

{% embed url="https://www.youtube-nocookie.com/embed/IHevXHniVvk" %}

## Eine Vorlage erstellen

{% content-ref url="creez-un-modele-daudit/" %}
[creez-un-modele-daudit](creez-un-modele-daudit/)
{% endcontent-ref %}

## Einen Fragebogen planen

{% content-ref url="planifier-un-audit.md" %}
[planifier-un-audit.md](planifier-un-audit.md)
{% endcontent-ref %}

## Fragebögen steuern

{% content-ref url="piloter-les-questionnaires.md" %}
[piloter-les-questionnaires.md](piloter-les-questionnaires.md)
{% endcontent-ref %}

## Einen Bericht exportieren

{% content-ref url="exporter-un-rapport.md" %}
[exporter-un-rapport.md](exporter-un-rapport.md)
{% endcontent-ref %}

## Fragebogentypen

{% content-ref url="types-de-questionnaires/" %}
[types-de-questionnaires](types-de-questionnaires/)
{% endcontent-ref %}

## FAQ

{% content-ref url="faq.md" %}
[faq.md](faq.md)
{% endcontent-ref %}

## Weiterführende Informationen

{% content-ref url="../la-gestion-des-risques/" %}
[la-gestion-des-risques](../la-gestion-des-risques/)
{% endcontent-ref %}
